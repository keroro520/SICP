...
;;	我的做法跟最后一楼AMS的一样 	楼上的做法虽然比我的麻烦, 但思想比我的巧妙, 且结合了之前的accumulator !
...
(define (augend exp) (if (null? (cdddr exp)) (caddr exp) (cons '+ (cddr exp))))		;@	处理多项参数, 一定要记得(cons '+ ...)
(define (multiplier exp) (cadr exp))
(define (multiplicand exp) (if (null? (cdddr exp)) (caddr exp) (cons '* (cddr exp))))	;@

(define (exponentiation? exp)
	(and (pair? exp) (eq? (car exp) '^)))
(define (base exp) (cadr exp))
(define (exponent exp) (if (null? (cdddr exp)) (caddr exp) (cons '^ (cddr exp))))	;@
